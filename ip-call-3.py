        ...
        try:
            if self.put_to_q:
                if self.main_self.ip_calls_instance.call_3_is_local:
                    self.main_self.secondary_slice_instance.ip_call_3_queue.put({"type":"slice","slice":slice})
                else:
                    self.main_self.final_slice_instance.ip_call_3_queue.put({"type": "slice", "slice": slice})
        except:
            error_message = traceback.format_exc()
            self.main_self.open_ip_calls_error_window(error_message)
            ...
            while (True):
                ...
                if len(self.ip_call_3_mp3_q) < self.packet_time:
                    while (len(self.ip_call_3_mp3_q) < self.packet_time):
                        chunk = self.ip_call_3_packet_queue.get()["packet"]
                        chunk_slice = AudioSegment(chunk, sample_width=2, frame_rate=48000, channels=2)
                        self.ip_call_3_mp3_q = self.ip_call_3_mp3_q + chunk_slice
                        time.sleep(0.020)
                    slice = self.ip_call_3_mp3_q[0:self.packet_time]
