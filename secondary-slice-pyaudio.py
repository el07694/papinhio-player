...
class Secondary_Slice_PyAudio:
    ...
    def __init__(self, main_self):
        try:
            ...
            # create process
            self.process_number = 104
            self.secondary_slice_pyaudio_mother_pipe, self.secondary_slice_pyaudio_child_pipe = Pipe()
            self.secondary_slice_pyaudio_queue = Queue()
            self.secondary_slice_pyaudio_emitter = Secondary_Slice_PyAudio_Emitter(self.secondary_slice_pyaudio_mother_pipe)
            self.secondary_slice_pyaudio_emitter.error_signal.connect(lambda error_message: self.main_self.open_secondary_slice_pyaudio_error_window(error_message))

            self.secondary_slice_pyaudio_emitter.start()
            self.secondary_slice_pyaudio_child_process = Secondary_Slice_PyAudio_Child_Proc(self.secondary_slice_pyaudio_child_pipe, self.secondary_slice_pyaudio_queue,self.main_self.condition, self.main_self.frame, self.main_self.quit_event)
            self.secondary_slice_pyaudio_child_process.start()
                ...
                if "process_number" in process:
                    if process["process_number"] == self.process_number:
                        self.main_self.manage_processes_instance.processes[counter][
                            "pid"] = self.secondary_slice_pyaudio_child_process.pid
            ...
            self.main_self.open_secondary_slice_pyaudio_error_window(error_message)

    def start_pyaudio(self):
        try:
            self.main_self.secondary_slice_instance.put_to_pyaudio = True
        except:
            error_message = traceback.format_exc()
            self.main_self.open_secondary_slice_pyaudio_error_window(error_message)
    ...
    def close(self):
        try:
            self.secondary_slice_pyaudio_queue.put({"type":"close"})
            try:
                if self.secondary_slice_pyaudio_child_process is not None:
                    self.secondary_slice_pyaudio_child_process.terminate()
                    self.secondary_slice_pyaudio_child_process.close()
            except Exception as e:
                pass
            try:
                if self.secondary_slice_pyaudio_emitter is not None:
                    self.secondary_slice_pyaudio_emitter.quit()
            except Exception as e:
                pass
...
class Secondary_Slice_PyAudio_Emitter(QThread):
    try:
