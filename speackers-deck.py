...
class Speackers_Deck:
    def __init__(self, main_self):
        try:
            ...
            self.main_self.ui.speackers_deck_hold_to_talk.setText("Πιέστε παρατεταμένα για να μιλήσετε")
            self.main_self.ui.speackers_deck_hold_to_talk.setStatusTip("Πιέστε παρατεταμένα για να μιλήσετε από το μικρόφωνο")
            self.main_self.ui.statusbar.showMessage("Πιέστε παρατεταμένα για να μιλήσετε από το μικρόφωνο")
            self.main_self.ui.speackers_deck_click_to_talk.setText("Πατήστε μία φορά για να μιλήσετε")
            self.main_self.ui.speackers_deck_click_to_talk.setStatusTip("Πατήστε μία φορά για να μιλήσετε από το μικρόφωνο")
            self.main_self.ui.statusbar.showMessage("Πατήστε μία φορά για να μιλήσετε από το μικρόφωνο")
            self.display_current_duration(0)
            self.display_volume_amplitude(0)

            # create process
            self.process_number = 92
            self.manage_speackers_deck_mother_pipe, self.manage_speackers_deck_child_pipe = Pipe()
            self.manage_speackers_deck_queue = Queue()
            self.manage_speackers_deck_emitter = Manage_Speackers_Deck_Emitter(self.manage_speackers_deck_mother_pipe)
            self.manage_speackers_deck_emitter.error_signal.connect(lambda error_message: self.main_self.open_speackers_deck_error_window(error_message))
            self.manage_speackers_deck_emitter.microphone_settings.connect(lambda settings: self.microphone_settings(settings))
            self.manage_speackers_deck_emitter.update_microphone_settings.connect(lambda settings: self.update_microphone_settings(settings))
            self.manage_speackers_deck_emitter.volume_amplitude.connect(lambda normalized_value: self.display_volume_amplitude(normalized_value))
            self.manage_speackers_deck_emitter.current_duration_milliseconds.connect(lambda duration: self.display_current_duration(duration))
            self.manage_speackers_deck_emitter.microphone_slice.connect(lambda slice: self.microphone_slice_ready(slice))

            self.manage_speackers_deck_emitter.start()
            self.manage_speackers_deck_child_process = Manage_Speackers_Deck_Child_Proc(
                self.manage_speackers_deck_child_pipe, self.manage_speackers_deck_queue,self.main_self.condition, self.main_self.frame, self.main_self.quit_event)
            self.manage_speackers_deck_child_process.start()
                ...
                if "process_number" in process:
                    if process["process_number"] == self.process_number:
                        self.main_self.manage_processes_instance.processes[counter][
                            "pid"] = self.manage_speackers_deck_child_process.pid
            ...
            self.main_self.open_speackers_deck_error_window(error_message)

    def microphone_slice_ready(self,slice):
        try:
            if self.put_to_q:
                self.main_self.final_slice_instance.speackers_deck_queue.put({"type":"slice","slice":slice})
                if self.main_self.ui.local_video_frame.isVisible():
    ...
    def display_volume_amplitude(self,normalized_value):
        try:
            ...
            frame_width = self.main_self.ui.speackers_deck_timeline_container_frame.geometry().width()
            stop_red = 255
            stop_green = int(255 * (1 - normalized_value))
            if (stop_green > 255):
                stop_green = 255
            normalized_value = int(frame_width * normalized_value)
            self.main_self.ui.speackers_deck_timeline_pick_frame.setGeometry(QtCore.QRect(0, 0, normalized_value, 16))
            self.main_self.ui.speackers_deck_timeline_pick_frame.setStyleSheet("QFrame{background-color:qlineargradient(spread:pad, x1:0, y1:0, x2:1, y2:0, stop:0 rgb(255, 255, 0), stop:1 rgb(" + str(stop_red) + ", " + str(stop_green) + ", 0))}")
            self.main_self.ui.speackers_deck_timeline_pick_frame.update()
        except:
            error_message = traceback.format_exc()
            self.main_self.open_speackers_deck_error_window(error_message)
    ...
    def display_current_duration(self,duration):
        try:
            ...
            self.main_self.ui.speackers_deck_time_label.setText(duration_human)
        except:
            error_message = traceback.format_exc()
            self.main_self.open_speackers_deck_error_window(error_message)
    ...
    def microphone_settings(self,settings):
        try:
            ...
            self.main_self.ui.speackers_deck_click_to_talk.clicked.connect(
                lambda state: self.click_to_talk_button_clicked())

            # hold to talk button
            self.main_self.ui.speackers_deck_hold_to_talk.pressed.connect(lambda : self.hold_button_pressed())
            self.main_self.ui.speackers_deck_hold_to_talk.released.connect(lambda : self.hold_button_released())
        except:
            error_message = traceback.format_exc()
            self.main_self.open_speackers_deck_error_window(error_message)

    def hold_button_pressed(self):
        try:
            self.deck_status = "playing"
            self.main_self.ui.speackers_deck_hold_to_talk.setText("Αφήστε το κουμπί όταν σταματήσετε να μιλάτε")
            self.main_self.ui.speackers_deck_click_to_talk.setStatusTip("Αφήστε το κουμπί (release) όταν σταματήσετε να μιλάτε για κλείσετε το μικρόφωνο.")
            self.main_self.ui.statusbar.showMessage("Αφήστε το κουμπί (release) όταν σταματήσετε να μιλάτε για κλείσετε το μικρόφωνο.")
            self.manage_speackers_deck_queue.put({"type": "new-status", "status": self.deck_status})
            if self.main_self.speackers_deck_secondary_instance.deck_status == "playing":
                self.main_self.speackers_deck_secondary_instance.deck_status = "stopped"
                self.main_self.speackers_deck_secondary_instance.manage_speackers_deck_secondary_queue.put({"type": "new-status", "status": self.main_self.speackers_deck_secondary_instance.deck_status})

        except:
            error_message = traceback.format_exc()
            self.main_self.open_speackers_deck_error_window(error_message)

    def hold_button_released(self):
        try:
            self.deck_status = "stopped"
            self.display_current_duration(0)
            self.display_volume_amplitude(0)
            self.main_self.ui.speackers_deck_click_to_talk.setText("Πατήστε μία φορά για να μιλήσετε")
            self.main_self.ui.speackers_deck_click_to_talk.setStatusTip(
                "Πατήστε μία φορά για να μιλήσετε από το μικρόφωνο")
            self.main_self.ui.statusbar.showMessage(
                "Πατήστε μία φορά για να μιλήσετε από το μικρόφωνο")
            self.main_self.ui.speackers_deck_hold_to_talk.setEnabled(True)
            self.main_self.ui.speackers_deck_hold_to_talk.setText("Πιέστε παρατεταμένα για να μιλήσετε")
            self.main_self.ui.speackers_deck_hold_to_talk.setStatusTip("Πιέστε παρατεταμένα για να μιλήσετε από το μικρόφωνο")
            self.main_self.ui.statusbar.showMessage("Πιέστε παρατεταμένα για να μιλήσετε από το μικρόφωνο")
    ...
    def click_to_talk_button_clicked(self):
        try:
            if self.deck_status == "stopped":
                self.deck_status = "playing"
                self.main_self.ui.speackers_deck_click_to_talk.setText("Πατήστε μία φορά όταν σταματήσετε να μιλάτε")
                self.main_self.ui.speackers_deck_click_to_talk.setStatusTip("Πατήστε μία φορά όταν σταματήσετε να μιλάτε για να κλείσετε το μικρόφωνο.")
                self.main_self.ui.statusbar.showMessage("Πατήστε μία φορά όταν σταματήσετε να μιλάτε για να κλείσετε το μικρόφωνο.")
                self.main_self.ui.speackers_deck_hold_to_talk.setEnabled(False)
                self.manage_speackers_deck_queue.put({"type":"new-status","status":self.deck_status})
                ...
                #    self.main_self.speackers_deck_secondary_instance.deck_status = "playing"
                #    self.main_self.speackers_deck_secondary_instance.manage_speackers_deck_secondary_queue.put({"type": "new-status", "status": self.main_self.speackers_deck_secondary_instance.deck_status})
    ...
    def create_menu_for_volume(self):
        try:
            self.menu_for_volume = QtWidgets.QMenu(self.main_self.ui.speackers_deck_volume)
            self.main_self.ui.speackers_deck_volume.setMenu(self.menu_for_volume)
    ...
    def volume_resetted(self):
        try:
            ...
            self.manage_speackers_deck_queue.put({"type": "volume", "value_base_100": 100})

            self.main_self.speackers_deck_secondary_instance.volume_slider.blockSignals(True)
            self.main_self.speackers_deck_secondary_instance.volume_slider.setValue(100)
            self.main_self.speackers_deck_secondary_instance.volume_slider.blockSignals(False)
            self.main_self.speackers_deck_secondary_instance.volume_label.setText("100/200")
        except:
            error_message = traceback.format_exc()
            self.main_self.open_speackers_deck_error_window(error_message)
    ...
    def volume_changed(self, slider_value):
        try:
            ...
            self.main_self.speackers_deck_secondary_instance.volume_label.setText(str(slider_value) + "/200")
            self.main_self.speackers_deck_secondary_instance.volume_slider.blockSignals(True)
            self.main_self.speackers_deck_secondary_instance.volume_slider.setValue(slider_value)
            self.main_self.speackers_deck_secondary_instance.volume_slider.blockSignals(False)
        except:
            error_message = traceback.format_exc()
            self.main_self.open_speackers_deck_error_window(error_message)
    ...
    def volume_released(self):
        try:
            ...
            self.manage_speackers_deck_queue.put({"type": "volume", "value_base_100": slider_value})
    ...
    def create_menu_for_pan(self):
        try:
            self.menu_for_pan = QtWidgets.QMenu(self.main_self.ui.speackers_deck_pan)
            self.main_self.ui.speackers_deck_pan.setMenu(self.menu_for_pan)
    ...
    def pan_resetted(self):
        try:
            ...
            self.manage_speackers_deck_queue.put({"type": "pan", "pan_value": 0})

            self.main_self.speackers_deck_secondary_instance.pan_slider.blockSignals(True)
            self.main_self.speackers_deck_secondary_instance.pan_slider.setValue(0)
            self.main_self.speackers_deck_secondary_instance.pan_slider.blockSignals(False)
            self.main_self.speackers_deck_secondary_instance.pan_label.setText("0")
        except:
            error_message = traceback.format_exc()
            self.main_self.open_speackers_deck_error_window(error_message)
    ...
    def pan_changed(self, slider_value):
        try:
            ...
            self.main_self.speackers_deck_secondary_instance.pan_slider.blockSignals(True)
            self.main_self.speackers_deck_secondary_instance.pan_slider.setValue(slider_value)
            self.main_self.speackers_deck_secondary_instance.pan_slider.blockSignals(False)
            self.main_self.speackers_deck_secondary_instance.pan_label.setText(str(slider_value))
        except:
            error_message = traceback.format_exc()
            self.main_self.open_speackers_deck_error_window(error_message)
    ...
    def pan_released(self):
        try:
            ...
            self.manage_speackers_deck_queue.put({"type": "pan", "pan_value": slider_value})
    ...
    def create_menu_for_normalize(self):
        try:
            self.menu_for_normalize = QtWidgets.QMenu(self.main_self.ui.speackers_deck_normalize)
            self.main_self.ui.speackers_deck_normalize.setMenu(self.menu_for_normalize)
    ...
    def normalize_changed(self, new_state):
        try:
            if new_state == QtCore.Qt.Unchecked:
                self.manage_speackers_deck_queue.put({"type": "is_normalized", "boolean_value": 0})
            else:
                self.manage_speackers_deck_queue.put({"type": "is_normalized", "boolean_value": 1})

            self.main_self.speackers_deck_secondary_instance.normalize_checkBox.blockSignals(True)
            self.main_self.speackers_deck_secondary_instance.normalize_checkBox.setCheckState(new_state)
            self.main_self.speackers_deck_secondary_instance.normalize_checkBox.blockSignals(False)
        except:
            error_message = traceback.format_exc()
            self.main_self.open_speackers_deck_error_window(error_message)
    ...
    def create_menu_for_filter(self):
        try:
            self.menu_for_filter = QtWidgets.QMenu(self.main_self.ui.speackers_deck_filter)
            self.main_self.ui.speackers_deck_filter.setMenu(self.menu_for_filter)
    ...
    def low_frequency_changed(self, low_frequency):
        try:
            ...
            self.main_self.speackers_deck_secondary_instance.high_frequency.setMinimum(low_frequency)
            self.main_self.speackers_deck_secondary_instance.low_frequency.blockSignals(True)
            self.main_self.speackers_deck_secondary_instance.low_frequency.setValue(low_frequency)
            self.main_self.speackers_deck_secondary_instance.low_frequency.blockSignals(False)
    ...
    def high_frequency_changed(self, high_frequency):
        try:
            ...
            self.main_self.speackers_deck_secondary_instance.low_frequency.setMaximum(high_frequency)
            self.main_self.speackers_deck_secondary_instance.high_frequency.blockSignals(True)
            self.main_self.speackers_deck_secondary_instance.high_frequency.setValue(high_frequency)
            self.main_self.speackers_deck_secondary_instance.high_frequency.blockSignals(False)
        except:
            error_message = traceback.format_exc()
            self.main_self.open_speackers_deck_error_window(error_message)
    ...
    def apply_filter_method(self, state):
        try:
            ...
            self.manage_speackers_deck_queue.put({"type": "low_frequency", "low_frequency_value": low_frequency})
            self.manage_speackers_deck_queue.put({"type": "high_frequency", "high_frequency_value": high_frequency})
    ...
    def reset_filter_method(self, state):
        try:
            ...
            self.manage_speackers_deck_queue.put({"type": "low_frequency", "low_frequency_value": 20})
            self.manage_speackers_deck_queue.put({"type": "high_frequency", "high_frequency_value": 20000})
    ...
    def close(self):
        try:
            if self.deck_status != "stopped":
                self.manage_speackers_deck_queue.put({"type":"new-status","status":"stopped"})
                self.deck_status = "stopped"
                time.sleep(0.125)
            self.manage_speackers_deck_queue.put({"type": "close"})
            time.sleep(0.8)
            try:
                if self.manage_speackers_deck_child_process is not None:
                    self.manage_speackers_deck_child_process.terminate()
                    self.manage_speackers_deck_child_process.close()
            except:
                pass
            try:
                if self.manage_speackers_deck_emitter is not None:
                    self.manage_speackers_deck_emitter.quit()
                    del self.manage_speackers_deck_emitter
            except:
                pass
...
class Manage_Speackers_Deck_Emitter(QThread):
    try:
...
class Manage_Speackers_Deck_Child_Proc(Process):
