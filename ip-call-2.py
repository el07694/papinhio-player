...
class Ip_Call_2:
    def __init__(self,main_self):
        try:
            ...
            # create process
            self.process_number = 107
            self.ip_call_2_mother_pipe, self.ip_call_2_child_pipe = Pipe()
            self.ip_call_2_queue = Queue(maxsize=100)
            self.ip_call_2_packet_queue = Queue()
            self.ip_call_2_emitter = Ip_Call_2_Emitter(self.ip_call_2_mother_pipe)
            self.ip_call_2_emitter.error_signal.connect(lambda error_message: self.main_self.open_ip_calls_error_window(error_message))
            self.ip_call_2_emitter.ip_call_2_ready.connect(lambda slice: self.ip_call_2_slice_ready(slice))
            self.ip_call_2_emitter.volume_amplitude.connect(lambda normalized_value: self.display_volume_amplitude(normalized_value))
            self.ip_call_2_emitter.current_duration_milliseconds.connect(lambda duration: self.display_current_duration(duration))

            self.ip_call_2_emitter.start()
            self.ip_call_2_child_process = Ip_Call_2_Child_Proc(self.ip_call_2_child_pipe, self.ip_call_2_queue,self.ip_call_2_packet_queue)
            self.ip_call_2_child_process.start()
                ...
                if "process_number" in process:
                    if process["process_number"] == self.process_number:
                        self.main_self.manage_processes_instance.processes[counter][
                            "pid"] = self.ip_call_2_child_process.pid
    ...
    def display_current_duration(self,duration):
        try:
            ...
            self.main_self.ui.ip_call_2_time_label.setText(duration_human)
        except:
            error_message = traceback.format_exc()
            self.main_self.open_ip_calls_error_window(error_message)
    ...
    def display_volume_amplitude(self,normalized_value):
        try:
            frame_width = self.main_self.ui.ip_call_2_timeline_container_frame.geometry().width()
            stop_red = 255
            stop_green = int(255 * (1 - normalized_value))
            if (stop_green > 255):
                stop_green = 255
            normalized_value = int(frame_width * normalized_value)
            self.main_self.ui.ip_call_2_timeline_pick_frame.setGeometry(QtCore.QRect(0, 0, normalized_value, 16))
            self.main_self.ui.ip_call_2_timeline_pick_frame.setStyleSheet("QFrame{background-color:qlineargradient(spread:pad, x1:0, y1:0, x2:1, y2:0, stop:0 rgb(255, 255, 0), stop:1 rgb(" + str(stop_red) + ", " + str(stop_green) + ", 0))}")
            self.main_self.ui.ip_call_2_timeline_pick_frame.update()
        except:
            error_message = traceback.format_exc()
            self.main_self.open_ip_calls_error_window(error_message)
    ...
    def init_buttons_and_sub_menus(self):
        try:
            ...
            self.main_self.ui.ip_call_2_forward_call.setFixedWidth(self.main_self.ui.ip_call_2_volume.sizeHint().width())
            self.main_self.ui.ip_call_2_dismiss.setFixedWidth(self.main_self.ui.ip_call_2_volume.sizeHint().width())
        except:
            error_message = traceback.format_exc()
            self.main_self.open_ip_calls_error_window(error_message)
    ...
    def create_menu_for_volume(self):
        try:
            self.menu_for_volume = QtWidgets.QMenu(self.main_self.ui.ip_call_2_volume)
            self.main_self.ui.ip_call_2_volume.setMenu(self.menu_for_volume)
    ...
    def volume_resetted(self):
        try:
            ...
            self.ip_call_2_queue.put({"type": "volume", "value_base_100": 100})
        except:
            error_message = traceback.format_exc()
            self.main_self.open_ip_calls_error_window(error_message)
    ...
    def volume_released(self):
        try:
            ...
            self.ip_call_2_queue.put({"type": "volume", "value_base_100": slider_value})
        except:
            error_message = traceback.format_exc()
            self.main_self.open_ip_calls_error_window(error_message)
    ...
    def create_menu_for_pan(self):
        try:
            self.menu_for_pan = QtWidgets.QMenu(self.main_self.ui.ip_call_2_pan)
            self.main_self.ui.ip_call_2_pan.setMenu(self.menu_for_pan)
    ...
    def pan_resetted(self):
        try:
            ...
            self.ip_call_2_queue.put({"type": "pan", "pan_value": 0})
        except:
            error_message = traceback.format_exc()
            self.main_self.open_ip_calls_error_window(error_message)
    ...
    def pan_released(self):
        try:
            ...
            self.ip_call_2_queue.put({"type": "pan", "pan_value": slider_value})
        except:
            error_message = traceback.format_exc()
            self.main_self.open_ip_calls_error_window(error_message)
    ...
    def create_menu_for_normalize(self):
        try:
            self.menu_for_normalize = QtWidgets.QMenu(self.main_self.ui.ip_call_2_normalize)
            self.main_self.ui.ip_call_2_normalize.setMenu(self.menu_for_normalize)
    ...
    def normalize_changed(self, new_state):
        try:
            if new_state == QtCore.Qt.Unchecked:
                self.ip_call_2_queue.put({"type": "is_normalized", "boolean_value": 0})
            else:
                self.ip_call_2_queue.put({"type": "is_normalized", "boolean_value": 1})
        except:
            error_message = traceback.format_exc()
            self.main_self.open_ip_calls_error_window(error_message)
    ...
    def create_menu_for_filter(self):
        try:
            self.menu_for_filter = QtWidgets.QMenu(self.main_self.ui.ip_call_2_filter)
            self.main_self.ui.ip_call_2_filter.setMenu(self.menu_for_filter)
    ...
    def apply_filter_method(self, state):
        try:
            ...
            self.ip_call_2_queue.put({"type": "low_frequency", "low_frequency_value": low_frequency})
            self.ip_call_2_queue.put({"type": "high_frequency", "high_frequency_value": high_frequency})
        except:
            error_message = traceback.format_exc()
            self.main_self.open_ip_calls_error_window(error_message)
    ...
    def reset_filter_method(self, state):
        try:
            ...
            self.high_frequency.setValue(20000)
            self.high_frequency.setMinimum(20)
            self.ip_call_2_queue.put({"type": "low_frequency", "low_frequency_value": 20})
            self.ip_call_2_queue.put({"type": "high_frequency", "high_frequency_value": 20000})
        except:
            error_message = traceback.format_exc()
            self.main_self.open_ip_calls_error_window(error_message)
    ...
    def close(self):
        try:
            self.ip_call_2_queue.put({"type": "close"})
            try:
                if self.ip_call_2_child_process is not None:
                    self.ip_call_2_child_process.terminate()
                    self.ip_call_2_child_process.close()
            except:
                pass
            try:
                if self.ip_call_2_emitter is not None:
                    self.ip_call_2_emitter.quit()
            except:
                pass
    ...
    def ip_call_2_slice_ready(self, slice):
        try:
            if self.put_to_q:
                # Select the correct queue based on whether the call is local or not
                target_queue = (
                    self.main_self.secondary_slice_instance.ip_call_2_queue
                    if self.main_self.ip_calls_instance.call_2_is_local
                    else self.main_self.final_slice_instance.ip_call_2_queue
                )
...
class Ip_Call_2_Emitter(QThread):
    try:
        error_signal = pyqtSignal(str)
        ip_call_2_ready = pyqtSignal(AudioSegment)
        ...
        try:
            while True:
                ...
                if data["type"] == "error":
                    ...
                    self.ip_call_2_ready.emit(data["slice"])
...
class Ip_Call_2_Child_Proc(Process):

    def __init__(self, to_emitter, from_mother,ip_call_2_packet_queue):
        try:
            super().__init__()
            self.daemon = False
            self.to_emitter = to_emitter
            self.data_from_mother = from_mother
            self.ip_call_2_packet_queue = ip_call_2_packet_queue
            self.volume = 100
            self.pan = 0
            self.normalize = 0
            ...
            self.ip_call_2_mp3_q = AudioSegment.empty()
        except:
            try:
    ...
    def run(self):
        try:
            while True:
                ...
                if len(self.ip_call_2_mp3_q) < self.packet_time:
                    while len(self.ip_call_2_mp3_q) < self.packet_time:
                        if not self.ip_call_2_packet_queue.empty():
                            # Fetch and convert packets to AudioSegment
                            packet_data = self.ip_call_2_packet_queue.get()["packet"]
                            chunk_slice = AudioSegment(packet_data, sample_width=2, frame_rate=48000, channels=2)
                            self.ip_call_2_mp3_q += chunk_slice
                        else:
                ...
                if len(self.ip_call_2_mp3_q) >= self.packet_time:
                    slice = self.ip_call_2_mp3_q[:self.packet_time]
                    self.ip_call_2_mp3_q = self.ip_call_2_mp3_q[self.packet_time:]
                else:
