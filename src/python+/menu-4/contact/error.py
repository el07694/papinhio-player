...
class Support_Ui_Dialog:
    ...
    def __init__(self,main_self,error_message):
        ...
        self.main_self.contact_error_window.hide()
        self.main_self.contact_error_window.show()

        self.main_self.ui_contact_error_window.details.setPlainText(str(error_message))

        self.main_self.ui_contact_error_window.restart_proccess.clicked.connect(lambda state:self.restart_proccess(state))
        self.main_self.ui_contact_error_window.ok.clicked.connect(lambda state:self.close_window(state))


        self.main_self.contact_error_window.closeEvent = lambda event:self.closeEvent(event)

    def restart_proccess(self,state):
        if self.main_self.contact_window_is_open:
            self.main_self.contact_window_support_code.close_window(None)
            self.main_self.contact_window_is_open = False

        self.main_self.contact_error_window.close()
        self.main_self.open_contact_window(None)
    ...
    def closeEvent(self,event):
        ...
        self.main_self.contact_error_window_is_open = False
        event.accept()