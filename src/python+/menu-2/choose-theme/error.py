...
class Support_Ui_Dialog:
    ...
    def __init__(self,main_self,error_message):
        ...
        self.main_self.choose_theme_error_window.setStyleSheet("*{font-family:""+self.main_self.default_font+"";font-size:"+self.main_self.default_font_size+"px;color:""+self.main_self.default_font_color+"";}QFrame{border:0px;}QDialog{background:""+self.main_self.default_background_color+""}QPushButton, QComboBox{background:""+self.main_self.default_buttons_background+"";color:""+self.main_self.default_buttons_font_color+""}")
        self.main_self.choose_theme_error_window.showMaximized()
        self.main_self.choose_theme_error_window.update()

        self.main_self.choose_theme_error_window.hide()
        self.main_self.choose_theme_error_window.show()

        self.main_self.ui_choose_theme_error_window.details.setPlainText(str(error_message))

        self.main_self.ui_choose_theme_error_window.restart_proccess.clicked.connect(lambda state:self.restart_proccess(state))
        self.main_self.ui_choose_theme_error_window.ok.clicked.connect(lambda state:self.close_window(state))

        
        self.main_self.choose_theme_error_window.closeEvent = lambda event:self.closeEvent(event)

    def restart_proccess(self,state):
        if self.main_self.change_theme_save_question_window_is_open:
        ...
        self.main_self.choose_theme_error_window.close()
        self.main_self.open_choose_theme_window(None)
