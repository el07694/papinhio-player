...
def import_retransmitions():
	...
	for retransmition_image in retransmition_images:
		shutil.copyfile(os.path.abspath("../../disket-box/test-contents/retransmitions/"+retransmition_image),os.path.abspath("../../disket-box/retransmitions-images/"+retransmition_image))

	retransmitions = []

	retransmitions.append((r'Πειραϊκή εκκλησία',r'https://www.peradio.com',r'dynamic',r'window.MP3jPLAYERS[0]["list"][0].mp3',r'https://impradio.bytemasters.gr/8002/LIVE',r'Πειραϊκή εκκλησία: Το ραδιόφωνο που μιλάει στην ψυχή μας.',os.path.abspath("../../disket-box/retransmitions-images/"+retransmition_images[0]),r'Πειραϊκή εκκλησία',10))
	retransmitions.append((r'Ραδιοφωνικός σταθμός: Συν πάσι τοις Αγίοις',r'https://synpasitoisagiois.radio12345.com/',r'dynamic',"document.getElementById('urladdress').innerHTML",r'https://freeuk20.listen2myradio.com/live.mp3?typeportmount=s1_19724_stream_430085248',r'Ραδιοφωνικός σταθμός Συν πάσι τοις Αγίοις',os.path.abspath("../../disket-box/retransmitions-images/"+retransmition_images[1]),r'Συν πάσι τοις Αγίοις',10))
