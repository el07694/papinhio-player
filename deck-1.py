...
class Deck_1:
    def __init__(self,main_self):
        try:
            ...
            # create process
            self.process_number = 98
            self.deck_1_mother_pipe, self.deck_1_child_pipe = Pipe()
            self.deck_1_queue = Queue()
            self.deck_1_emitter = Deck_1_Emitter(self.deck_1_mother_pipe)
            self.deck_1_emitter.error_signal.connect(lambda error_message: self.main_self.open_deck_1_error_window(error_message))
            self.deck_1_emitter.deck_1_ready.connect(lambda slice: self.deck_1_slice_ready(slice))
            self.deck_1_emitter.volume_amplitude.connect(lambda normalized_value: self.display_volume_amplitude(normalized_value))
            self.deck_1_emitter.current_duration_milliseconds.connect(lambda duration: self.display_current_duration(duration))
            self.deck_1_emitter.deck_finished.connect(lambda: self.deck_finished())
            self.deck_1_emitter.chunk_number_answer.connect(lambda chunk_number: self.main_self.player_list_instance.chunk_number_answer("deck_1",chunk_number))
            self.deck_1_emitter.url_check_result.connect(lambda result, retransmition: self.url_check_result(result, retransmition))
            self.deck_1_emitter.fade_out_start.connect(lambda: self.fade_out_start())


            self.deck_1_emitter.start()
            self.deck_1_child_process = Deck_1_Child_Proc(self.deck_1_child_pipe, self.deck_1_queue,self.main_self.condition, self.main_self.frame, self.main_self.quit_event)
            self.deck_1_child_process.start()
                ...
                if "process_number" in process:
                    if process["process_number"] == self.process_number:
                        self.main_self.manage_processes_instance.processes[counter][
                            "pid"] = self.deck_1_child_process.pid
    ...
    def test_deck(self):
        try:
            ...
            item = database_functions.read_retransmition(2)
            item["duration_milliseconds"] = 1000*60
    ...
    def deck_finished(self):
        try:
            ...
            if self.main_self.player_list_instance.auto_dj_mode:
                if not (self.main_self.player_list_instance.player_list_settings["player_fade_out"] and
                        self.main_self.player_list_instance.player_list_settings["player_fade_in"]):
            ...
            if self.deck_status == "stopped":
                if self.main_self.player_list_instance.player_list_play_deck_queue.qsize()>0:
                    ...
                    if "retransmition" in relative_type:
                        self.main_self.deck_1_instance.play_retransmition = True
                    # 1. stop deck 1
                    self.main_self.deck_1_instance.stop_button_clicked(disable_extra_plays=False)
    ...
    def stop_button_clicked(self,disable_extra_plays=True):
        try:
            ...
            #print(disable_extra_plays)
            if disable_extra_plays:
                self.main_self.player_list_instance.time_collection_play_in_progress = False
                self.main_self.player_list_instance.playlist_play_in_progress = False
            self.deck_status = "stopped"
            self.deck_1_queue.put({"type":"new-status","status":self.deck_status})
            self.display_current_duration(0)
            self.display_volume_amplitude(0)
            icon1 = QtGui.QIcon()
            icon1.addPixmap(QtGui.QPixmap(":/rest-icons/assets/icons/rest-icons/play-song.png"), QtGui.QIcon.Normal,QtGui.QIcon.Off)
    ...
    def deck_timeline_slider_moved(self):
        try:
            ...
            if (self.deck_status != "stopped"):
                ...
                self.deck_1_queue.put({"type": "duration_changed", "chunk_number": chunk_number})
            else:
    ...
    def volume_resetted(self):
        try:
            ...
            self.deck_1_queue.put({"type": "volume", "value_base_100": 100})
    ...
    def volume_released(self):
        try:
            ...
            self.deck_1_queue.put({"type": "volume", "value_base_100": slider_value})
    ...
    def pan_resetted(self):
        try:
            ...
            self.deck_1_queue.put({"type": "pan", "pan_value": 0})
        except:
            error_message = traceback.format_exc()
            self.main_self.open_deck_1_error_window(error_message)
    ...
    def pan_released(self):
        try:
            ...
            self.deck_1_queue.put({"type": "pan", "pan_value": slider_value})
        except:
            error_message = traceback.format_exc()
            self.main_self.open_deck_1_error_window(error_message)
    ...
    def normalize_changed(self, new_state):
        try:
            if new_state == QtCore.Qt.Unchecked:
                self.deck_1_queue.put({"type": "is_normalized", "boolean_value": 0})
            else:
                self.deck_1_queue.put({"type": "is_normalized", "boolean_value": 1})
        except:
            error_message = traceback.format_exc()
            self.main_self.open_deck_1_error_window(error_message)
    ...
    def apply_filter_method(self, state):
        try:
            ...
            self.deck_1_queue.put({"type": "low_frequency", "low_frequency_value": low_frequency})
            self.deck_1_queue.put({"type": "high_frequency", "high_frequency_value": high_frequency})
        except:
            error_message = traceback.format_exc()
            self.main_self.open_deck_1_error_window(error_message)
    ...
    def reset_filter_method(self, state):
        try:
            ...
            self.high_frequency.setValue(20000)
            self.high_frequency.setMinimum(20)
            self.deck_1_queue.put({"type": "low_frequency", "low_frequency_value": 20})
            self.deck_1_queue.put({"type": "high_frequency", "high_frequency_value": 20000})
        except:
            error_message = traceback.format_exc()
            self.main_self.open_deck_1_error_window(error_message)
    ...
    def load_item(self,item):
        try:
            ...
            self.deck_1_queue.put({"type": "load", "item": self.item})
        except:
            error_message = traceback.format_exc()
            self.main_self.open_deck_1_error_window(error_message)
    ...
    def check_radio_url(self,item):
        try:
            ...
            if url_option == "dynamic" or url_option == "youtube-stream":
                ...
                self.deck_1_queue.put({"type": "check-radio-url", "retransmition": self.tmp_item})
        except:
            error_message = traceback.format_exc()
            print(error_message)
            self.main_self.open_deck_1_error_window(error_message)
    ...
    def close(self):
        try:
            if self.deck_status == "playing":
                ...
                self.deck_1_queue.put({"type":"new-status","status":self.deck_status})
                time.sleep(0.125)
            self.deck_1_queue.put({"type": "close"})
            try:
                if self.deck_1_child_process is not None:
                    self.deck_1_child_process.terminate()
                    self.deck_1_child_process.close()
            except:
                pass
            try:
                if self.deck_1_emitter is not None:
                    self.deck_1_emitter.quit()
            except:
                pass
    ...
    def deck_1_slice_ready(self,slice):
        try:
            if self.put_to_q:
                self.main_self.final_slice_instance.deck_1_queue.put({"type":"slice","slice":slice})
        except:
            error_message = traceback.format_exc()
            self.main_self.open_deck_1_error_window(error_message)
...
class Deck_1_Child_Proc(Process):
