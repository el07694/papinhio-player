...
class Deck_2:
    def __init__(self,main_self):
        try:
            ...
            # hide web_frame
            self.main_self.ui.deck_2_web.hide()

            self.main_self.ui.deck_2_play_or_pause.setStatusTip("Αναπαραγωγή ή παύση αρχείου ή αναμετάδοσης στο deck 2. Πατήστε για αναπαραγωγή")

            self.main_self.ui.deck_2_label_name.setText("Deck 2:")

            # create process
            self.process_number = 99
            self.deck_2_mother_pipe, self.deck_2_child_pipe = Pipe()
            self.deck_2_queue = Queue()
            self.deck_2_emitter = Deck_2_Emitter(self.deck_2_mother_pipe)
            self.deck_2_emitter.error_signal.connect(lambda error_message: self.main_self.open_deck_2_error_window(error_message))
            self.deck_2_emitter.deck_2_ready.connect(lambda slice: self.deck_2_slice_ready(slice))
            self.deck_2_emitter.volume_amplitude.connect(lambda normalized_value: self.display_volume_amplitude(normalized_value))
            self.deck_2_emitter.current_duration_milliseconds.connect(lambda duration: self.display_current_duration(duration))
            self.deck_2_emitter.deck_finished.connect(lambda: self.deck_finished())
            self.deck_2_emitter.chunk_number_answer.connect(lambda chunk_number: self.main_self.player_list_instance.chunk_number_answer("deck_2",chunk_number))
            self.deck_2_emitter.url_check_result.connect(lambda result, retransmition: self.url_check_result(result, retransmition))
            self.deck_2_emitter.fade_out_start.connect(lambda: self.fade_out_start())


            self.deck_2_emitter.start()
            self.deck_2_child_process = Deck_2_Child_Proc(self.deck_2_child_pipe, self.deck_2_queue,self.main_self.condition, self.main_self.frame, self.main_self.quit_event)
            self.deck_2_child_process.start()
                ...
                if "process_number" in process:
                    if process["process_number"] == self.process_number:
                        self.main_self.manage_processes_instance.processes[counter][
                            "pid"] = self.deck_2_child_process.pid
            ...
            self.main_self.open_deck_2_error_window(error_message)
    ...
    def url_check_result(self, result, retransmition):
        try:
            ...
            if result == False:
                self.play_retransmition = False
                self.main_self.open_deck_2_web_error_window(retransmition)
                # if auto dj on continue auto dj
                if self.main_self.player_list_instance.player_list_settings["auto_dj"]:
                    self.main_self.player_list_instance.dj_retransmition_error(retransmition)
            else:
    ...
    def test_deck(self):
        try:
            #item = database_functions.read_church_new(1)

            #item = database_functions.read_record_file(1)

            #item = database_functions.read_like_today(3)

            #item = database_functions.read_sound_file(100)

            #item = database_functions.read_greek_lessons(2)

            #item = database_functions.read_sound_clip(10)

            #item = database_functions.read_station_logo(2)

            #item = database_functions.read_time_collection(1)[1][1]

            #item = database_functions.read_weather_new(2)

            #item = database_functions.read_history(5)

            #item = database_functions.read_documentary(5)

            #item = database_functions.read_sport_new(5)

            item = database_functions.read_retransmition(1)
            item["duration_milliseconds"] = 1000*60*30
            item["duration_human"] = convert_time_function.convert_duration_from_milliseconds_to_human(item["duration_milliseconds"])
            self.load_item(item)
        except:
            error_message = traceback.format_exc()
            self.main_self.open_deck_2_error_window(error_message)
    ...
    def deck_finished(self):
        try:
            ...
            self.main_self.ui.deck_2_play_or_pause.setIcon(icon1)
            ...
            if self.main_self.player_list_instance.auto_dj_mode:
                ...
                if "playlist_play_in_progress" in dir(self.main_self.player_list_instance):
                    if self.main_self.player_list_instance.playlist_play_in_progress:
                        ...
                        if item is not None:
                            # 1. stop deck 2
                            self.main_self.deck_2_instance.stop_button_clicked(disable_extra_plays=False)

                            # 2. load item to deck_1
                            self.main_self.deck_2_instance.load_item(item)
                    ...
                    if self.main_self.player_list_instance.time_collection_play_in_progress:
                        ...
                        if "restore_deck" in dir(self.main_self.player_list_instance):
                            if self.main_self.player_list_instance.restore_deck:
                                if "time_item" in self.item["type"]:
            ...
            if self.deck_status == "stopped":
                if self.main_self.player_list_instance.player_list_play_deck_queue.qsize()>0:
                    play_or_prepare,item = self.main_self.player_list_instance.player_list_play_deck_queue.get()
                    relative_type = item["type"]
                    if "retransmition" in relative_type:
                        self.main_self.deck_2_instance.play_retransmition = True
                    # 1. stop deck 2
                    self.main_self.deck_2_instance.stop_button_clicked(disable_extra_plays=False)
    ...
    def display_current_duration(self,duration):
        try:
            ...
            if self.item is not None:
                self.main_self.ui.deck_2_duration.setText(duration_human +"/" + str(self.item["duration_human"]))

                self.main_self.ui.deck_2_timeslider.blockSignals(True)
                self.main_self.ui.deck_2_timeslider.setValue(int(duration))
                self.main_self.ui.deck_2_timeslider.blockSignals(False)


            else:
                self.main_self.ui.deck_2_duration.setText("00:00:00/-")
        except:
            error_message = traceback.format_exc()
            self.main_self.open_deck_2_error_window(error_message)
    ...
    def display_volume_amplitude(self,normalized_value):
        try:
            ...
            frame_width = self.main_self.ui.deck_2_timeline_container_frame.geometry().width()
            stop_red = 255
            stop_green = int(255 * (1 - normalized_value))
            if (stop_green > 255):
                stop_green = 255
            normalized_value = int(frame_width * normalized_value)
            self.main_self.ui.deck_2_timeline_pick_frame.setGeometry(QtCore.QRect(0, 0, normalized_value, 16))
            self.main_self.ui.deck_2_timeline_pick_frame.setStyleSheet("QFrame{background-color:qlineargradient(spread:pad, x1:0, y1:0, x2:1, y2:0, stop:0 rgb(255, 255, 0), stop:1 rgb(" + str(stop_red) + ", " + str(stop_green) + ", 0))}")
            self.main_self.ui.deck_2_timeline_pick_frame.update()
        except:
            error_message = traceback.format_exc()
            self.main_self.open_deck_2_error_window(error_message)
    ...
    def init_buttons_and_sub_menus(self):
        try:
            # deck stop button
            self.main_self.ui.deck_2_stop.clicked.connect(lambda state:self.stop_button_clicked())

            # deck play or pause button
            self.main_self.ui.deck_2_play_or_pause.clicked.connect(lambda state:self.play_or_pause_clicked())

            # deck timeline
            self.main_self.ui.deck_2_timeslider.sliderReleased.connect(lambda:self.deck_timeline_slider_moved())
            ...
            self.main_self.ui.deck_2_play_or_pause.setFixedWidth(self.main_self.ui.deck_2_volume.sizeHint().width())
            self.main_self.ui.deck_2_stop.setFixedWidth(self.main_self.ui.deck_2_volume.sizeHint().width())
        except:
            error_message = traceback.format_exc()
            self.main_self.open_deck_2_error_window(error_message)
    ...
    def stop_button_clicked(self,disable_extra_plays=True):
        try:
            if disable_extra_plays:
                ...
                self.main_self.player_list_instance.playlist_play_in_progress = False
            self.deck_status = "stopped"
            self.deck_2_queue.put({"type":"new-status","status":self.deck_status})
            self.display_current_duration(0)
            self.display_volume_amplitude(0)
            icon1 = QtGui.QIcon()
            icon1.addPixmap(QtGui.QPixmap(":/rest-icons/assets/icons/rest-icons/play-song.png"), QtGui.QIcon.Normal,QtGui.QIcon.Off)
            ...
            self.main_self.ui.deck_2_timeslider.blockSignals(True)
            self.main_self.ui.deck_2_timeslider.setValue(0)
            self.main_self.ui.deck_2_timeslider.blockSignals(False)
    ...
    def play_or_pause_clicked(self):
        try:
            ...
            if self.deck_status == "playing":
                ...
                self.main_self.ui.deck_2_play_or_pause.setStatusTip(
                    "Αναπαραγωγή ή παύση αρχείου ή αναμετάδοσης στο deck 2. Πατήστε για παύση")
    ...
    def deck_timeline_slider_moved(self):
        try:
            value = self.main_self.ui.deck_2_timeslider.value()
            if (self.deck_status != "stopped"):
                # calculate new chunk_number
                chunk_number = round(value / 125)
                self.deck_2_queue.put({"type": "duration_changed", "chunk_number": chunk_number})
            else:
                self.main_self.ui.deck_2_timeslider.setProperty("value", 0)
        except:
            error_message = traceback.format_exc()
            self.main_self.open_deck_2_error_window(error_message)
    ...
    def create_menu_for_volume(self):
        try:
            self.menu_for_volume = QtWidgets.QMenu(self.main_self.ui.deck_2_volume)
            self.main_self.ui.deck_2_volume.setMenu(self.menu_for_volume)
    ...
    def volume_resetted(self):
        try:
            ...
            self.deck_2_queue.put({"type": "volume", "value_base_100": 100})
    ...
    def volume_released(self):
        try:
            ...
            self.deck_2_queue.put({"type": "volume", "value_base_100": slider_value})
    ...
    def create_menu_for_pan(self):
        try:
            self.menu_for_pan = QtWidgets.QMenu(self.main_self.ui.deck_2_pan)
            self.main_self.ui.deck_2_pan.setMenu(self.menu_for_pan)
    ...
    def pan_resetted(self):
        try:
            ...
            self.deck_2_queue.put({"type": "pan", "pan_value": 0})
        except:
            error_message = traceback.format_exc()
            self.main_self.open_deck_2_error_window(error_message)
    ...
    def pan_released(self):
        try:
            ...
            self.deck_2_queue.put({"type": "pan", "pan_value": slider_value})
        except:
            error_message = traceback.format_exc()
            self.main_self.open_deck_2_error_window(error_message)
    ...
    def create_menu_for_normalize(self):
        try:
            self.menu_for_normalize = QtWidgets.QMenu(self.main_self.ui.deck_2_normalize)
            self.main_self.ui.deck_2_normalize.setMenu(self.menu_for_normalize)
    ...
    def normalize_changed(self, new_state):
        try:
            if new_state == QtCore.Qt.Unchecked:
                self.deck_2_queue.put({"type": "is_normalized", "boolean_value": 0})
            else:
                self.deck_2_queue.put({"type": "is_normalized", "boolean_value": 1})
        except:
            error_message = traceback.format_exc()
            self.main_self.open_deck_2_error_window(error_message)
    ...
    def create_menu_for_filter(self):
        try:
            self.menu_for_filter = QtWidgets.QMenu(self.main_self.ui.deck_2_filter)
            self.main_self.ui.deck_2_filter.setMenu(self.menu_for_filter)
    ...
    def apply_filter_method(self, state):
        try:
            ...
            self.deck_2_queue.put({"type": "low_frequency", "low_frequency_value": low_frequency})
            self.deck_2_queue.put({"type": "high_frequency", "high_frequency_value": high_frequency})
        except:
            error_message = traceback.format_exc()
            self.main_self.open_deck_2_error_window(error_message)
    ...
    def reset_filter_method(self, state):
        try:
            ...
            self.high_frequency.setValue(20000)
            self.high_frequency.setMinimum(20)
            self.deck_2_queue.put({"type": "low_frequency", "low_frequency_value": 20})
            self.deck_2_queue.put({"type": "high_frequency", "high_frequency_value": 20000})
        except:
            error_message = traceback.format_exc()
            self.main_self.open_deck_2_error_window(error_message)
    ...
    def load_item(self,item):
        try:
            ...
            # 2. change deck 2 title
            self.main_self.ui.deck_2_label_name.setText("Deck 2: "+item["title"])

            # 3. change deck 2 image
            if "image_path" in item:
                image_path = item["image_path"]
                pixmap = QtGui.QPixmap(image_path)
                self.main_self.ui.deck_2_image.setPixmap(pixmap)
                relative_type = item["type"]
            else:
                relative_type = item["type"]
                if relative_type == "station_logo" or relative_type == "station_logos":
                    ...
                    self.main_self.ui.deck_2_image.setPixmap(QtGui.QPixmap(":/rest-icons/assets/icons/rest-icons/deck-image.png"))
            ...
            self.main_self.ui.deck_2_image.setStatusTip(image_title)
            ...
            if "time_item" in relative_type:
                ...
                if append:
                    ...
                    self.item["duration_human"] = convert_time_function.convert_duration_from_milliseconds_to_human(
                        new_duration_milliseconds)

            # 4. change deck 2 duration
            self.display_current_duration(0)

            # 5. change deck 2 amplitude
            self.display_volume_amplitude(0)

            # 6. set slider max value
            self.main_self.ui.deck_2_timeslider.setMaximum(int(self.item["duration_milliseconds"]))
            self.main_self.ui.deck_2_timeslider.setSingleStep(500)
            self.main_self.ui.deck_2_timeslider.setPageStep(1000)
            self.main_self.ui.deck_2_timeslider.setProperty("value", 0)
            if self.item["type"] != "retransmitions":
                self.main_self.ui.deck_2_timeslider.setEnabled(True)
            else:
                self.main_self.ui.deck_2_timeslider.setEnabled(False)

            #7. set deck volume
            self.volume_slider.blockSignals(True)
            self.volume_slider.setValue(self.item["volume"])
            self.volume_label.setText(str(self.item["volume"])+"/200")
            ...
            self.pan_label.setText(str(self.item["pan"]))
            ...
            self.low_frequency.setValue(self.item["low_frequency"])
            self.low_frequency.setMaximum(self.item["high_frequency"])
            self.high_frequency.setValue(self.item["high_frequency"])
            self.high_frequency.setMinimum(self.item["low_frequency"])
            ...
            self.deck_2_queue.put({"type": "load", "item": self.item})
        except:
            error_message = traceback.format_exc()
            self.main_self.open_deck_2_error_window(error_message)
            ...
            if url_option == "dynamic" or url_option == "youtube-stream":
                try:
                    if "web" in dir(self):
                        ...
                        del self.web
                    self.clearLayout(self.main_self.ui.horizontalLayout_13)

                except Exception as e:
                    error_message = str(traceback.format_exc())
                    print(error_message)

                self.main_self.ui.deck_2_web.show()
                self.web = QWebEngineView(self.main_self.ui.deck_2_web)
                ...
                self.deck_2_queue.put({"type": "check-radio-url", "retransmition": self.tmp_item})
        except:
            error_message = traceback.format_exc()
            self.main_self.open_deck_2_error_window(error_message)
        ...
        try:
            if (self.page_loaded == False):
                if (ok):
                    ...
                    self.page_loaded = True
                    self.main_self.open_deck_2_web_error_window(self.tmp_item)
        except:
            error_message = traceback.format_exc()
            self.main_self.open_deck_2_error_window(error_message)
    ...
    def close(self):
        try:
            if self.deck_status == "playing":
                ...
                self.deck_2_queue.put({"type":"new-status","status":self.deck_status})
                time.sleep(0.125)
            self.deck_2_queue.put({"type": "close"})
            try:
                if self.deck_2_child_process is not None:
                    self.deck_2_child_process.terminate()
                    self.deck_2_child_process.close()
            except:
                pass
            try:
                if self.deck_2_emitter is not None:
                    self.deck_2_emitter.quit()
            except:
                pass
    ...
    def deck_2_slice_ready(self,slice):
        try:
            if self.put_to_q:
                self.main_self.final_slice_instance.deck_2_queue.put({"type":"slice","slice":slice})
        except:
            error_message = traceback.format_exc()
            self.main_self.open_deck_2_error_window(error_message)
...
class Deck_2_Emitter(QThread):
    try:
        error_signal = pyqtSignal(str)
        deck_2_ready = pyqtSignal(AudioSegment)
        ...
        try:
            while True:
                ...
                if data["type"] == "error":
                    ...
                    self.deck_2_ready.emit(data["slice"])
...
class Deck_2_Child_Proc(Process):
        ...
        try:
            ...
            if data is not None:
                if data["type"] == "volume":
                    ...
                    #in case of time_item with append calculate new total_duration_milliseconds
                    if "time_item" in self.item["type"]:
                        time_collection = self.item["time_collection"]
                        #append = int(time_collection["append"])
                        #if append:
                        #    extra_item = time_collection["append_item"]
                        #    extra_duration_milliseconds = extra_item["duration_milliseconds"]
                        #    self.total_duration_milliseconds += extra_duration_milliseconds
                    try:
                        self.read_mp3.join()
                        del self.read_mp3
                    except:
            ...
            if self.item is not None:
                if self.item["type"] != "retransmitions":
                    if self.audio_segment == "not-ready":
                        ...
                        #self.to_emitter.send({"type": "slice", "slice": AudioSegment.empty()})
                        return None
    ...
    def check_retransmition(self, stream_url):
        try:
            ...
            if os.path.exists("deck_2_retransmition_check.mp3"):
                os.remove("deck_2_retransmition_check.mp3")

            self.p1 = Popen([self.ffmpeg_path, '-y', '-t', '10', '-loglevel', 'quiet', '-i', stream_url,'deck_2_retransmition_check.mp3'], stdout=PIPE, stdin=PIPE,stderr=PIPE, shell=True)
            self.p1.wait()
    ...
    def read_retransmition_slice(self):
        try:
            ...
            if os.path.exists("deck_2_retransmition.mp3"):
                if "retransmition_file" not in dir(self):
                    self.retransmition_file = open("deck_2_retransmition.mp3", "rb")
    ...
    def stop_retransmition(self):
        try:
            try:
                if "deck_2_ffmpeg_retransmition" in dir(self):
                    self.deck_2_ffmpeg_retransmition.communicate(b"q")
                    self.deck_2_ffmpeg_retransmition.terminate()
                    self.deck_2_ffmpeg_retransmition.wait()
            except:
                pass
            ...
            if os.path.exists("deck_2_retransmition.mp3"):
                os.remove("deck_2_retransmition.mp3")
        except:
            error_message = traceback.format_exc()
            self.to_emitter.send({"type": "error", "error_message": error_message})
    ...
    def start_retransmition(self):
        try:
            ...
            self.deck_2_ffmpeg_retransmition = Popen([self.ffmpeg_path, '-y', '-loglevel', 'quiet', '-i', self.item["stream_url"],'deck_2_retransmition.mp3'], stdin=PIPE, stdout=PIPE, stderr=PIPE)
        except:
            error_message = traceback.format_exc()
            self.to_emitter.send({"type": "error", "error_message": error_message})
